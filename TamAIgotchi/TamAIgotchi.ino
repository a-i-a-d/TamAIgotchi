...
void sendTest() {
  if ((WiFi.status() == WL_CONNECTED)) {
    ...
    http.begin(client, "http://" SERVER_IP "/v1/chat/completions");  // HTTP
    http.addHeader("Content-Type", "application/json");

    Serial.print("[HTTP] POST...\n");
    // start connection and send HTTP header and body
    int httpCode = http.POST("{\"model\":\"gpt-4\", \"messages\": [{\"role\": \"user\", \"content\": \"Say this is a test!\"}]}");
    ...
    if (httpCode > 0) {
      ...
      if (httpCode == HTTP_CODE_OK) {
        ...
        DeserializationError error = deserializeJson(json_reply, payload);
        if (error) {
          Serial.print(F("deserializeJson() failed: "));
          Serial.println(error.f_str());
          return;
        }
        const char* id = json_reply["id"];
        const char* reply = json_reply["choices"][0]["message"]["content"];
        Serial.print("Reply: ");
